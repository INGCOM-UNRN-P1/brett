---
title: "Guía de Extensión y Creación de Plugins: brett"
subtitle: "Manual de integración, desarrollo de extensiones y uso de la API Python de brett"
author: "Cátedra de Algoritmos y Programación"
date: "2026-08-31"
---

(manual-brett-plugins)=
# Guía de Extensión y Plugins: brett

````{abstract}
Esta guía técnica detalla cómo desarrollar extensiones, crear nuevos plugins e integrar programáticamente **`brett`** en herramientas de evaluación, entornos de integración continua (CI/CD) o scripts docentes.
````

---

(manual-brett-plugins-arquitectura)=
## 1. Arquitectura de Extensión

`brett` provee una arquitectura modular desacoplada basada en puntos de entrada (Entry Points) estándar de Python (`[project.entry-points]`) o interfaces de inyección funcional:

- **Mecanismo de Extensión Principal**: `Extensiones y Reglas Personalizadas para brett`.
- **Punto de Entrada Oficial**: `brett.plugins`.
- **Formato de Comunicación**: Estructuras de datos serializables JSON / Pydantic models.

---

(manual-brett-plugins-tutorial)=
...
class CustomPlugin:
    """Plugin de extensión para brett."""
    name = "custom_rule"
...
Para que `brett` descubra y cargue automáticamente tu plugin, agregalo en tu `pyproject.toml`:

````{code-block} toml
[project.entry-points."brett.plugins"]
mi_plugin = "mi_paquete.modulo:MiPlugin"
````

### Paso 3: Instalar y Verificar el Plugin

Instalá tu extensión en modo editable y comprobá que `brett` la reconozca:
...
(manual-brett-plugins-sdk)=
## 3. Conexión Programática mediante la API Python

Podés importar y ejecutar `brett` directamente desde scripts de Python sin invocar subprocesos:
...
resultado = brett.ejecutar_analisis(
...
(manual-brett-plugins-ci)=
## 4. Integración en Pipelines de CI/CD (GitHub Actions / GitLab CI)

Podés integrar `brett` en tus flujos automatizados de Git para bloquear entregas que no cumplan los requisitos de cátedra:
...
jobs:
  auditoria:
    ...
    steps:
      ...
      - name: Instalar brett
        run: pip install -e ./brett
        
      - name: Ejecutar Auditoría
        run: brett check src/ include/ --json > reporte.json
````

---

(manual-brett-plugins-ejercicios)=
